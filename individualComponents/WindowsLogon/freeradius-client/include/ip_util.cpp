...
uint32_t rc_get_ipaddr (char *host)
{
	...
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_get_ipaddr: couldn't resolve hostname: %s", host);
		return (uint32_t)0;
	}
...
const char *rc_ip_hostname (uint32_t h_ipaddr)
{
	...
	if ((hp = rc_gethostbyaddr ((char *) &n_ipaddr, sizeof (struct in_addr),
			    AF_INET)) == NULL) {
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_ip_hostname: couldn't look up host by addr: %08lX", h_ipaddr);
	}
...
rc_own_hostname(char *hostname, int len)
{
	...
	if (uname(&uts) < 0)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_own_hostname: couldn't get own hostname");
		return -1;
	}
...
uint32_t rc_own_ipaddress(rc_handle *rh)
{
	...
	if (!rh->this_host_ipaddr) {
		...
		{
			strncpy_s(hostname,HOST_NAME_LENGTH_WITH_NULL_TERMINATOR, rc_conf_str(rh, "bindaddr"), sizeof(hostname));
			hostname[sizeof(hostname) - 1] = '\0';
		}
		if ((rh->this_host_ipaddr = rc_get_ipaddr (hostname)) == 0) {
			rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_own_ipaddress: couldn't get own IP address");
			return 0;
		}
	}
...
uint32_t rc_own_bind_ipaddress(rc_handle *rh)
{
	...
	if (rh->this_host_bind_ipaddr == NULL)
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_own_bind_ipaddress: out of memory");
	if (rc_conf_str(rh, "bindaddr") == NULL || strcmp(rc_conf_str(rh, "bindaddr"), "*") == 0) 
	{
		rval = INADDR_ANY;
	} 
	else 
	{
		...
		{
			rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_own_ipaddress: couldn't get IP address from bindaddr");
			rval = INADDR_ANY;
		}
	}
	if (rh->this_host_bind_ipaddr != NULL)
...
rc_get_srcaddr(struct sockaddr *lia, struct sockaddr *ria)
{
	...
	if (temp_sock == -1) 
	{
		...
		error_buffer[4095] = '\0';
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_get_srcaddr: socket: %s",error_buffer );
		return -1;
	}
	...
	if (connect(temp_sock, ria, SA_LEN(ria)) != 0) 
	{
		...
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_get_srcaddr: connect: %s", error_buffer);
	...
	if (getsockname(temp_sock, lia, &namelen) != 0) 
	{
		...
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_get_srcaddr: getsockname: %s", error_buffer);
		closesocket(temp_sock);
		return -1;
	}
