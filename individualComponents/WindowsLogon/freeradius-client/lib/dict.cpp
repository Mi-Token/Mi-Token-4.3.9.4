...
int rc_read_dictionary (rc_handle *rh, const char *filename)
{
	...
	fopen_s (&dictfd, filename, "r");
	if (dictfd == NULL)
	{
		strerror_s(error_buffer, 4096, errno);
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: couldn't open dictionary %s: %s", filename, error_buffer);
		return -1;
	}
	...
	while (fgets (buffer, sizeof (buffer), dictfd) != NULL)
	{
		...
		if (strncmp (buffer, "ATTRIBUTE", 9) == 0)
		{
			...
			if (sscanf_s (buffer, "%s%s%s%s%s", dummystr, namestr, valstr, typestr, optstr) < 4)
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid attribute on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid name length on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			if (!isdigit (*valstr))
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid value on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			value = atoi (valstr);
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid type on line %d of dictionary %s",
					 line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			if (optstr[0] != '\0') {
				...
				for (cp1 = optstr; cp1 != NULL; cp1 = cp) {
					...
					if (dvend == NULL) {
						rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: unknown Vendor-Id %s on line %d of dictionary %s",
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: out of memory");
				fclose(dictfd);
				return -1;
			}
			strcpy_s(attr->name, NAME_LENGTH_WITH_NULL_TERMINATOR, namestr);
			attr->value = value;
			attr->type = type;
		...
		{
			/* Read the VALUE line */
			if (sscanf_s (buffer, "%s%s%s%s", dummystr, attrstr,
				    namestr, valstr) != 4)
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid value entry on line %d of dictionary %s",
				line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid attribute length on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			strcpy_s(dval->attrname, NAME_LENGTH_WITH_NULL_TERMINATOR, attrstr);
			strcpy_s(dval->name, NAME_LENGTH_WITH_NULL_TERMINATOR, namestr);
			dval->value = value;
			...
			if (sscanf_s (buffer, "%s%s", dummystr, namestr) != 2)
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid include entry on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			ifilename = namestr;
		...
		{
			/* Read the VALUE line */
			if (sscanf_s (buffer, "%s%s%s", dummystr, attrstr, valstr) != 3)
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid Vendor-Id on line %d of dictionary %s", line_no, filename);
				fclose(dictfd);
				return -1;
			}
			...
			strcpy_s(dvend->vendorname,NAME_LENGTH_WITH_NULL_TERMINATOR, attrstr);
...
int rc_initialize_dictionary (rc_handle *rh)
{
	int retval = 0;

	if((retval = rc_add_dictionary(rh,"username","string","1")) < 0)
	{
		retval = -1;
	}

	if((retval = rc_add_dictionary(rh,"password","string","2")) < 0)
	{
		retval =  -1;
	}

	if((retval = rc_add_dictionary(rh,"service_type","integer","6")) < 0)
	{
		retval =  -1;
	}
...
int rc_add_dictionary (rc_handle *rh, const char* namestr, const char  *typestr, const char *valstr)
{
	DICT_ATTR      *attr;
	DICT_VENDOR    *dvend;
	int             value;
	...
	if (strlen(namestr) > NAME_LENGTH)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid name length");
		return -1;
	}

	if (!isdigit (*valstr))
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_dictionary: invalid value.");
		return -1;
	}
...
rc_dict_findvend(const rc_handle *rh, const char *vendorname)
{
	...
	for (vend = rh->dictionary_vendors; vend != NULL; vend = vend->next)
		...
		if (strcmp(vend->vendorname, vendorname) == 0)
			return vend;
	return NULL;
}
