...
char *rc_getifname(rc_handle *rh, char *tty)
{
	...
	if ((fd = open(tty, O_RDWR|O_NDELAY)) < 0) {
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_getifname: can't open %s: %s", tty, strerror(errno));
		return NULL;
	}
#endif
	...
	if (strlen(rh->ifname) < 1) {
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_getifname: can't get attached interface of %s: %s", tty, strerror(errno));
		close(fd);
		return NULL;
	}
#elif linux
	if (ioctl(fd, SIOCGIFNAME, rh->ifname) < 0) {
		rc_event_log(EVENTLOG_ERROR_TYPE, FREE_RADIUS_GENERAL_CATEGORY, FREE_RADIUS_MESSAGE, L"rc_getifname: can't ioctl %s: %s", tty, strerror(errno));
		close(fd);
		return NULL;
	}
#else
...
{
  sprintf_s (rh->buf1, "%08lX%04X", (unsigned long int) time (NULL), (unsigned int)1/* getpid ()*/);
  return rh->buf1;
}
...
rc_new(void)
{
	...
	if (rh == NULL) 
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_new: out of memory");
		return NULL;
    }
