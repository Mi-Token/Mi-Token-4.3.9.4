...
#define RADIUS_SERVER_SETTINGS "SOFTWARE\\Mi-Token"
/*
...
static int set_option_str(const char *filename, int line, OPTION *option, const char *p)
{
	if (p) 
	{
		option->val = (void *) _strdup(p);
		if (option->val == NULL) 
		{
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"read_config: out of memory");
			return -1;
		}
	} 
	else 
...
static int set_option_int(const char *filename, int line, OPTION *option, const char *p)
{
	...
	if (p == NULL) 
	{
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: bogus option value", filename, line);
		return -1;
	}
...
static int set_option_srv(const char *filename, int line, OPTION *option, const char *p)
{
	...
	if (p_dupe == NULL) {
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: Invalid option or memory failure", filename, line);
		return -1;
	}
	...
	p_pointer = strtok_s(p_dupe, ", \t", &p_save);
	...
	if ((q = strchr(p_pointer,':')) != NULL) {
		*q = '\0';
		...
		if((s = strchr(q,':')) != NULL) {
			*s = '\0';
			s++;
			serv->secret[serv->max] = _strdup(s);			
	...
	if(q && strlen(q) > 0) {
		...
		if (!strcmp(option->name,"authserver"))
			...
			rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: no default port for %s", filename, line, option->name);
			if (option->val == NULL) {
				free(p_dupe);
				free(serv);
			}
	...
	serv->name[serv->max] = _strdup(p_pointer);
...
static int set_option_auo(const char *filename, int line, OPTION *option, const char *p)
{
	...
	if (p_dupe == NULL) {
		rc_event_log(EVENTLOG_WARNING_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: bogus option value", filename, line);
		return -1;
	}
	...
	*iptr = 0;
	...
	if (!strncmp(p_pointer, "local", 5))
		...
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: auth_order: unknown keyword: %s", filename, p);
		free(p_dupe);
		return -1;
	}

	//p_pointer = strtok_r(NULL, ", \t", &p_save);
	p_pointer = strtok_s(NULL, ", \t", &p_save);
	...
	if (p_pointer && (*p_pointer != '\0')) {
		if ((*iptr & AUTH_RADIUS_FST) && !strcmp(p_pointer, "local"))
			...
			*iptr = (*iptr) | AUTH_RADIUS_SND;
			...
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: auth_order: unknown or unexpected keyword: %s", filename, p);
			free(p_dupe);
			return -1;
		}
	}
...
int rc_add_config(rc_handle *rh, const char *option_name, const char *option_val, const char *source, const int line)
{
	...
	{
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"ERROR: unrecognized option: %s", option_name);
		return -1;
	}

	if (option->status != ST_UNDEF) 
	{
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"ERROR: duplicate option: %s", option_name);
		return -1;
	}
	...
	switch (option->type) 
	{
		...
		case OT_AUO:
			...
			break;
		default:
			rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_config: impossible case branch!");
			abort();
	}
	return 0;
}
...
rc_config_init(rc_handle *rh)
{
	...
	{
            rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_config_init: out of memory");
			rc_destroy(rh);
            return NULL;
    }
	...
	authservers = (SERVER *)malloc(sizeof(SERVER));
	acctservers = (SERVER *)malloc(sizeof(SERVER));

	if(authservers == NULL || acctservers == NULL)
	{
                rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_config_init: error initializing server structs");
		rc_destroy(rh);
                return NULL;
	}
...
int rc_set_option(rc_handle *rh, char* name, const char* value)
{
	int result = 0;
	OPTION *option;

	if ((option = find_option(rh, name, OT_ANY)) == NULL) 
	{
		result = -1;
	}
	else if (option->status != ST_UNDEF) 
	{
		result = -1;
	}
	else
	{
		...
		{
			case OT_STR:
				if (set_option_str(NULL, NULL, option, value) < 0) 
				{
					result = -1;
				}
				break;
			case OT_INT:
				if (set_option_int(NULL, NULL, option, value) < 0) 
				{
 					result = -1;
				}
				break;
			case OT_SRV:
				if (set_option_srv(NULL, NULL, option, value) < 0) 
				{
 					result = -1;
				}
				break;
			case OT_AUO:
				if (set_option_auo(NULL, NULL, option, value) < 0) 
				{
 					result = -1;
				}
				break;
...
rc_handle* rc_read_registry()
{
    ...
    if (rh->config_options == NULL) 
	{
        rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_config: out of memory");
		rc_destroy(rh);
        return NULL;
    }
	...
	std::vector<RadiusServer> servers = RegistryHelper::RetrieveRadiusServers(RADIUS_SERVER_SETTINGS);
	...
	for(std::vector<RadiusServer>::iterator itr = servers.begin(); itr <servers.end(); itr++)
	{
		CT2CA pszServerName ( itr->serverName);
		CT2CA pszServerSecret ( itr->serverSecret);

		/*Server config format-> Server Name : Server Port : Server Secret*/
		oss << pszServerName << ":" << itr->serverPort << ":" << pszServerSecret;

		if(rc_set_option(rh, "authserver",oss.str().c_str()) <0)
		{
		 	return NULL;
		}
		oss.str("");
	...
	if(rc_set_option(rh, "radius_timeout","5") < 0)
	{
	 	return NULL;
	}

	if(rc_set_option(rh, "radius_retries","5") < 0)
	{
	 	return NULL;
	}

	if(rc_set_option(rh, "radius_deadtime","5") < 0)
	{
	 	return NULL;
	}

	/*Always guess a random sequence number.*/
	if(rc_set_option(rh, "seqfile","cannot_be_found.txt") < 0)
	{
	 	return NULL;
	}
...
rc_read_config(char *filename)
 {
	...
	fopen_s(&configfd, filename, "r");
	if (configfd == NULL)
	{	
		strerror_s(error_buffer, 4096, errno);
		error_buffer[4095] = '\0';
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_read_config: can't open %s: %s", filename, error_buffer);
		rc_destroy(rh);
		return NULL;
	}
	...
	while ((fgets(buffer, sizeof(buffer), configfd) != NULL))
	{
		...
		if ((pos = strcspn(p, "\t ")) == 0) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: bogus format: %s", filename, line, p);
			fclose(configfd);
			rc_destroy(rh);
			return NULL;
		}
		...
		if ((option = find_option(rh, p, OT_ANY)) == NULL) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: unrecognized keyword: %s", filename, line, p);
			fclose(configfd);
			rc_destroy(rh);
			return NULL;
		}
		...
		if (option->status != ST_UNDEF) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: line %d: duplicate option line: %s", filename, line, p);
			fclose(configfd);
			rc_destroy(rh);
			return NULL;
		}
...
char *rc_conf_str(rc_handle *rh, char *optname)
{
	...
	if (option != NULL) {
		...
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_conf_str: unkown config option requested: %s", optname);
		abort();
		return NULL;
	}
}
...
int rc_conf_int(rc_handle *rh, char *optname)
{
	...
	if (option != NULL) {
		...
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_conf_int: unkown config option requested: %s", optname);
		abort();
		return 0;
	}
}
...
SERVER *rc_conf_srv(rc_handle *rh, char *optname)
{
	...
	if (option != NULL) {
		...
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_conf_srv: unkown config option requested: %s", optname);
		abort();
		return NULL;
	}
}
...
int test_config(rc_handle *rh, char *filename)
{
	...
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: no authserver specified", filename);
		return -1;
	}

	if (rc_conf_int(rh, "radius_timeout") <= 0)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: radius_timeout <= 0 is illegal", filename);
		return -1;
	}
	if (rc_conf_int(rh, "radius_retries") <= 0)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: radius_retries <= 0 is illegal", filename);
		return -1;
	}
	if (rc_conf_int(rh, "radius_deadtime") < 0)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: radius_deadtime is illegal", filename);
		return -1;
	}
#if 0
	...
	if (stat(file, &st) == 0)
	{
		if (!S_ISREG(st.st_mode)) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: not a regular file: %s", filename, file);
			return -1;
		}
	} else {
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"%s: file not found: %s", filename, file);
		return -1;
	}
...
int rc_find_server (rc_handle *rh, char *server_name, uint32_t *ip_addr, char *secret)
{
	...
	if( (authservers = rc_conf_srv(rh, "authserver")) != NULL ) 
	{
		for( i = 0; i < authservers->max; i++ )
		{
			    ...
			    (authservers->secret[i] != NULL) )
			{
				...
				std::string decoded = base64_decode(authservers->secret[i]);
				...
				strncpy_s (secret, MAX_SECRET_LENGTH_WITH_NULL_TERMINATOR,decrypted_value/* authservers->secret[i]*/, (size_t) len);
	...
	if( (acctservers = rc_conf_srv(rh, "acctserver")) != NULL ) 
	{
		for( i = 0; i < acctservers->max; i++ )
		{
			    ...
			    (acctservers->secret[i] != NULL) )
			{
				...
				strncpy_s (secret,MAX_SECRET_LENGTH_WITH_NULL_TERMINATOR, acctservers->secret[i], (size_t) len);
				secret[MAX_SECRET_LENGTH] = '\0';
				return 0;
			}
		}
	...
	fopen_s(&clientfd, rc_conf_str(rh, "servers"), "r");
	if (clientfd == NULL)
	{
		strerror_s(error_buffer, 4096, errno);
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_find_server: couldn't open file: %s: %s", error_buffer, rc_conf_str(rh, "servers"));
		return -1;
	}
	...
	while (fgets (buffer, sizeof (buffer), clientfd) != NULL)
	{
		...
		strncpy_s (hostnm, AUTH_ID_LEN_WITH_NULL_TERMINATOR, h, (size_t) len);
		...
		strncpy_s (secret,MAX_SECRET_LENGTH_WITH_NULL_TERMINATOR, s, (size_t) len);
		secret[MAX_SECRET_LENGTH] = '\0';
	...
	if (result == 0)
	{
		...
		rc_event_log(EVENTLOG_ERROR_TYPE,  PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_find_server: couldn't find RADIUS server %s in %s",	 server_name, rc_conf_str(rh, "servers"));
		return -1;
	}
	return 0;
}
