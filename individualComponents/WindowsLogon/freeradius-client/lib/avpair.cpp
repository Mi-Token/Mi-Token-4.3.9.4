...
int rc_avpair_assign (VALUE_PAIR *vp, void *pval, int len)
{
	...
	case PW_TYPE_STRING:
		...
		if (len > AUTH_STRING_LEN) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_assign: bad attribute length");
			return -1;
		}
	...
	default:
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_assign: unknown attribute %d", vp->type);
		return -1;
	}
	return 0;
}
...
VALUE_PAIR *rc_avpair_new (const rc_handle *rh, int attrid, void *pval, int len, int vendorpec)
{
	...
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_new: unknown attribute %d", attrid);
		return NULL;
	}
	if (vendorpec != 0 && rc_dict_getvend(rh, vendorpec) == NULL)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_new: unknown Vendor-Id %d", vendorpec);
		return NULL;
	}
	if ((vp = (VALUE_PAIR *)malloc (sizeof (VALUE_PAIR))) != NULL)
	{
		strncpy_s (vp->name, NAME_LENGTH_WITH_NULL_TERMINATOR, pda->name, sizeof (vp->name));
		vp->attribute = attrid;
		vp->next = NULL;
		vp->type = pda->type;
	...
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_new: out of memory");
	}
...
VALUE_PAIR * rc_avpair_gen(const rc_handle *rh, VALUE_PAIR *pair, unsigned char *ptr, int length, int vendorpec)
{
	...
	if (length < 2) {
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received attribute with invalid length");
		goto shithappens;
	}
	attrlen = ptr[1];
	...
	if (attribute == PW_VENDOR_SPECIFIC) 
	{
		if (attrlen < 4) 
		{
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received VSA attribute with invalid length");
			goto shithappens;
		}
		memcpy(&lvalue, ptr, 4);
		...
		if (rc_dict_getvend(rh, vendorpec) == NULL) 
		{
			...
			rc_event_log(EVENTLOG_WARNING_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received VSA attribute with unknown Vendor-Id %d", vendorpec);
			return pair;
		}
		/* Process recursively */
	...
	if (attr == NULL) 
	{
		...
		{
			sprintf_s(hex, 3, "%2.2X", x_ptr[0]);
			strcat_s(buffer,(AUTH_STRING_LEN * 2) + 1, hex);
		}
		if (vendorpec == 0) 
		{
			rc_event_log(EVENTLOG_WARNING_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received unknown attribute %d of length %d: 0x%s", attribute, attrlen + 2, buffer);
		} 
		else 
		{
			rc_event_log(EVENTLOG_WARNING_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received unknown VSA attribute %d, vendor %d of length %d: 0x%s", attribute & 0xffff, VENDOR(attribute), attrlen + 2, buffer);
		}
		goto shithappens;
	}
	...
	if (rpair == NULL) 
	{
		rc_event_log(EVENTLOG_WARNING_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: out of memory");
		goto shithappens;
	}
	...
	rpair->next = pair;
	pair = rpair;
	strcpy_s(pair->name, NAME_LENGTH_WITH_NULL_TERMINATOR, attr->name);
	...
	case PW_TYPE_INTEGER:
		if (attrlen != 4) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received INT attribute with invalid length");
			goto shithappens;
		}
	case PW_TYPE_IPADDR:
		if (attrlen != 4) {
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: received IPADDR attribute with invalid length");
			goto shithappens;
		}
	...
	default:
		rc_event_log(EVENTLOG_WARNING_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_gen: %s has unknown type", attr->name);
		goto shithappens;
	}
	return pair;
...
void rc_avpair_insert (VALUE_PAIR **a, VALUE_PAIR *p, VALUE_PAIR *b)
{
	...
	if (b->next != NULL)
	{
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_insert: value pair (0x%p) next ptr. (0x%p) not NULL", b, b->next);
		abort ();
	}
...
int rc_avpair_parse (const rc_handle *rh, char *buffer, VALUE_PAIR **first_pair)
{
	...
	while (*buffer != '\n' && *buffer != '\0')
	{
		...
		case PARSE_MODE_NAME:		/* Attribute Name */
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_parse: unknown attribute");
				if (*first_pair) {
		...
		case PARSE_MODE_EQUAL:		/* Equal sign */
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_parse: missing or misplaced equal sign");
				if (*first_pair) {
					rc_avpair_free(*first_pair);
					*first_pair = NULL;
				}
		...
		case PARSE_MODE_VALUE:		/* Value */
			...
			{
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_parse: out of memory");
				if (*first_pair) {
					rc_avpair_free(*first_pair);
					*first_pair = NULL;
				}
			...
			case PW_TYPE_STRING:
				strcpy_s(pair->strvalue, AUTH_STRING_LEN_WITH_NULL_TERMINATOR, valstr);
			...
			case PW_TYPE_INTEGER:
				...
				else
				{
					if ((dval = rc_dict_findval (rh, valstr))
						== NULL)
					{
						rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_parse: unknown attribute value: %s", valstr);
						if (*first_pair) {
							rc_avpair_free(*first_pair);
							*first_pair = NULL;
						}
			...
			default:
				rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_parse: unknown attribute type %d", pair->type);
				if (*first_pair) {
					rc_avpair_free(*first_pair);
					*first_pair = NULL;
				}
...
int rc_avpair_tostr (const rc_handle *rh, VALUE_PAIR *pair, char *name, int ln, char *value, int lv)
{
	...
	*name = *value = '\0';
	...
	if (!pair || pair->name[0] == '\0') {
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_tostr: pair is NULL or empty");
		return -1;
	}

	/*TODO Verify buffer size.*/
	strncpy_s(name, NAME_LENGTH, pair->name, (size_t) ln);
	...
	case PW_TYPE_STRING:
		...
		while (*ptr != '\0')
		{
			if (!(isprint (*ptr)))
			{
				sprintf_s(buffer, 32, "\\%03o", *ptr);
				strncat_s(value, lv, buffer, (size_t) lv);
				lv -= 4;
				if (lv < 0) break;
			}
			else
			{
				strncat_s(value, lv, (char *)ptr, 1);
				lv--;
				if (lv < 0) break;
			}
			ptr++;
	...
	case PW_TYPE_INTEGER:
		...
		if (dval != NULL)
		{
			strncpy_s(value, lv, dval->name, (size_t) lv-1);
		}
		else
		{
			sprintf_s(buffer, 32, "%ld", (long int)pair->lvalue);
			strncpy_s(value, lv, buffer, (size_t) lv);
		}
		break;

	case PW_TYPE_IPADDR:
		inad.s_addr = htonl(pair->lvalue);
		strncpy_s (value, lv, inet_ntoa (inad), (size_t) lv-1);
		break;

	case PW_TYPE_DATE:
		struct tm timeinfo;
		gmtime_s (&timeinfo, (time_t *) & pair->lvalue);
		//strftime (buffer, sizeof (buffer), "%m/%d/%y %H:%M:%S", gmtime ((time_t *) & pair->lvalue));
		strftime (buffer, sizeof (buffer), "%m/%d/%y %H:%M:%S", &timeinfo);
		strncpy_s(value, lv, buffer, lv-1);
		break;

	default:
		rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_tostr: unknown attribute type %d", pair->type);
...
*
*/
char* rc_avpair_log(rc_handle *rh, VALUE_PAIR *pair)
{
	size_t len, nlen;
	VALUE_PAIR *vp;
	...
	for (vp = pair; vp != NULL; vp = vp->next) 
	{
		...
		if (cp == NULL) 
		{
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_log: can't allocate memory");
			return NULL;
		}
		sprintf_s(cp + len, nlen - len, "%-32s = '%s'\n", name, value);
		rh->ppbuf = cp;
		len = nlen - 1;
	}
...
VALUE_PAIR *rc_avpair_readin(const rc_handle *rh, FILE *input)
{
	...
	while (fgets(buffer, sizeof(buffer), input) != NULL)
	{
		...
		{
			rc_event_log(EVENTLOG_ERROR_TYPE, PACKET_GENERATION_CATEGORY, RC_AVPAIR_GEN, L"rc_avpair_readin: malformed attribute: %s", buffer);
			rc_avpair_free(vp);
			return NULL;
		}
	}
